...
public class XaeroIntegration {
    ...
    public static void syncPlayerPositions(List<PlayerData> players) {
        ...
        try {
            ...
            ClaimViz.LOGGER.debug("ClaimViz: Xaero player sync failed: {}", e.getMessage());
        }
    }
    ...
    public static void syncClaimWaypoints(List<ClaimRect> claims) {
        ...
        try {
            ...
            for (ClaimRect claim : claims) {
                ...
                ClaimViz.LOGGER.info("ClaimViz: adding waypoint '{}' at ({}, 64, {})", name, cx, cz);
            ...
            ClaimViz.LOGGER.warn("ClaimViz: Xaero waypoint sync failed: {} — {}", e.getClass().getSimpleName(), e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private static Object getCurrentWaypointSet() throws Exception {
        // Chain: XaeroMinimapSession → getMinimapProcessor() → getSession() [MinimapSession]
        //        → getWorldManager() → getCurrentWorld() → getCurrentWaypointSet()
        Object xaeroSession = getOrInitSession();
        if (xaeroSession == null) return null;
    ...
    private static String waypointName(ClaimRect claim) {
        if ("Administrator".equals(claim.owner())) return "Admin Claim";
        return claim.owner() + "'s Claim";
    }

    private static String waypointInitials(ClaimRect claim) {
        if ("Administrator".equals(claim.owner())) return "AD";
    ...
    private static int waypointColor(ClaimRect claim, String selfName) {
        ...
        if (selfName != null && selfName.equalsIgnoreCase(claim.owner())) return COLOR_OWN;
        return COLOR_OTHER;
    }
    ...
    /** Converts SquareMap dim key "minecraft_overworld" → RegistryKey for minecraft:overworld. */
